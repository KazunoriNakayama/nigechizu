...
function onDocumentMouseMove( event ){
	...
	vector = vector.subSelf( camera.position ).normalize();
...
//3Dの地形ジオメトリー作成
	...
	mesh = new THREE.SceneUtils.createMultiMaterialObject( new THREE.PlaneGeometry( meshW, meshH, gridU+2, gridV+2 ), 
							     [ new THREE.MeshPhongMaterial( { color: 0xffffff, map: geoTexture } ) ] 
							     );
...
//3Dのパス図形の作成
	...
	for( var i = 0; i < network3D.Pathes.length+100; i++ ){
		...
		for( var j = 0; j < 10; j++ ){
			geometry.vertices.push( new THREE.Vertex( new THREE.Vector3( base.x, base.y, base.z ) ) );
		}
	...
	for( var i = 0; i < tData.length ; i++ ){
		...
		for( var j = 0; j < tData[ i ].length ; j++ ){
			...
			for( var k = 0; k < tData[ i ][ j ][ 1 ].length; k++){
				Geometrys[ j ].vertices.push( new THREE.Vertex( new THREE.Vector3( base.x, base.y, base.z ) ) );
			}
		}
